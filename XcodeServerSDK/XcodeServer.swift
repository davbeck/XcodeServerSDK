...
// MARK: URLSession delegate implementation
...
public extension XcodeServer {
	...
	private func verifyAPIVersion(_ response: HTTPURLResponse) -> Error? {
    ...
    */
	@discardableResult
	internal func sendRequest(with method: HTTP.Method, endpoint: XcodeServerEndpoints.Endpoint, params: [String: String]?, query: [String: String]?, body: [String:Any]?, portOverride: Int? = nil, completion: @escaping HTTP.Completion) -> URLSessionTask? {
		if let request = self.endpoints.createRequest(method: method, endpoint: endpoint, params: params, query: query, body: body, portOverride: portOverride) {
